	...
	if(file_exists($filename)){
		...
		foreach($welive_onlines AS $key => $value){
			if(array_key_exists($userid, $value['user'])){
				...
				if($item == 'isonline'){
					$welive_onlines[$key]['user'][$userid]['isbusy'] = '0'; //更改了在線狀態時, 清除重來忙碌狀態為不忙
				}
...
function checkbusy($userid){
	...
	foreach($welive_onlines AS $key => $value){
		if(array_key_exists($userid, $value['user'])){
			if(!$value['user'][$userid]['isbusy']) return 0; //目前客服不忙
			$groupid = $key;
			break;
		}
	}
